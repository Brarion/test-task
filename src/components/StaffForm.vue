<template>
	<v-form ref="form" v-model="valid">
		<v-text-field v-model="form.fio" label="ФИО сотрудника" required :rules="rules.fio"></v-text-field>
		<v-text-field
...
export default {
	...
	data() {
		return {
			...
			rules: {
				fio: [
					fio => !!fio || "Обязательное поле",
					fio =>
						(fio && fio.trim().split(" ").length >= 2 && fio.trim().split(" ").length <= 3) ||
						"Фамилия и имя должны быть заполнены полностью"
				],
				pass_ser: [
					pass_ser => !!pass_ser || "Обязательное поле",
					pass_ser => (pass_ser && pass_ser.length === 4) || "Должно быть 4 цифры"
				],
				pass_no: [
					pass_no => !!pass_no || "Обязательное поле",
					pass_no => (pass_no && pass_no.length === 6) || "Должно быть 6 цифр"
				],
		...
		save() {
			if (this.$props.staff.id) {
				this.$props.saveStaff({
					...this.form,
					pass_dt: moment(this.form.pass_dt).toISOString(),
